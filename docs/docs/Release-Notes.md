...
- Removed `GetDetectionType()` from CPP Component API.
- Removed `detection_type` from Python Component API.
- Removed `getDetectionType()` from Java Component API.
...
- Each component's `descriptor.json` now requires an `algorithm.trackType` field. This is used by the Workflow Manager
  to determine the kind of tracks that may be generated by the component (e.g. `FACE`, `TEXT`, `CLASS`, etc.). This now
  used in place of the component API calls that were used to get the detection type. 
