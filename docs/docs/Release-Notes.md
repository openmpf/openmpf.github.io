...
- As part of the text tagging update, the `TAGS` outputs are now separated by semicolons `;` rather than commas `,` to align with the `TRIGGER_WORDS` and `TRIGGER_WORDS_OFFSET` output patterns. Commas are now used to group each set of `TRIGGER_WORDS` with its respective `TAG` output.
