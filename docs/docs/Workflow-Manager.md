...
3. The **Workflow Manager**, which allows for the creation of jobs and manages the flow through active components
...
There are 3 general functional areas in the Workflow Manager.

1. The **Controllers** are the primary entry point, accepting REST requests which trigger actions by the workflow manager.
...
There are two different databases used by the Workflow Manager.
...
2. A Redis database for storing transient data that is only necessary while the job is being run.
Some of this data is used to generate the persisted output, and some is duplicating persisted
data to avoid race conditions. This data includes:
...
The controllers are all located at [https://github.com/openmpf/openmpf/tree/master/trunk/workflow-manager/src/main/java/org/mitre/mpf/mvc/controller](https://github.com/openmpf/openmpf/tree/master/trunk/workflow-manager/src/main/java/org/mitre/mpf/mvc/controller ).
...
There are a number of controllers that serve a limited number of functions.
...
| BootoutController | Handles bootouts when a second session is opened |
...
| TimeoutController | Manages timeouts |
...
3. Registration and unregistration of components -- Components must be registered to be included in pipelines
...
Markup files are copies of the initial media input to a job with any detections visually highlighted in the image or video frames. The **MarkupController** can provide lists of available Markup files, or it can download a specific file.
...
For more information on nodes, please read the [Node Configuration and Status](Admin-Manual#node-configuration-and-status) guide.
...
For more information on pipelines, please read the User Guide on how to [Create Custom Pipelines](User-Guide#create-custom-pipelines).

## Job Management

The request to create a job begins at the [`JobController`](#JobController). From there, it is transformed and passed through multiple flows on its way to the component services which process it, then return it for output generation.
...
The **Media Inspection Route** splits a single job with multiple media inputs into separate messages, one for each piece of media. For each piece of media, it collects metadata about the media, including MIME type, duration, frame rate, and EXIF orientation data.

### Job Router Route

The **Job Router** uses the pipeline's flow to create the messages that are sent to the components. For large media files, it splits the job into smaller processing tasks by assigning a start and end point to be processed.  It compiles properties for the job, media, and algorithm, and determines the next component that needs to be invoked. It then marshals the job into serialized protobuf format and sends the messages off to the component for processing.

Unlike Job Creation, Media Retriever, and Media Inspection, the Job Router may be invoked multiple times, as future routes redirect back to the Job Router so that the job can be processed by the next component in the pipeline.

### Detection Response Route

The **Detection Response Route** is the re-entry point to the Workflow Manager. It unmarshals the protobuf responses, converts them into the Track and Detection objects used within the Workflow Manager, and stores them in the Redis database. Over multiple calls to the DetectionResponseProcessor, all data is eventually added into the transient Job object.

### Stage Response Aggregation Route

The **Stage Response Aggregation Route** is the exit point for the response processors. They wait until all responses have been retrieved, then invoke the Job Router Route to see if any additional processing needs to be done.
